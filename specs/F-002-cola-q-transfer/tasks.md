...
- [x] T1: sv5 — adaptadores `infrastructure/azure/` (`cola_cliente.py`,
...
- [x] T2: sv5 — split del pipeline en fases (ampliación 1):
...
- [x] T3: sv5 — `config/settings.py` con las claves de storage opcionales
...
- [x] T4: sv5 — `interface_adapters/queue/transfer_consumer.py`: handler
...
- [x] T5: sv5 — `main.py`: composición (lock único + pipeline compartido,
...
- [x] T6: sv4 — adaptadores `infrastructure/azure/` (mismos tres ficheros,
...
- [x] T7: sv4 — `TransferQueuePublisher` +
...
- [x] T8: sv4 — endpoint `POST /api/aprobar/encolar` con fallback síncrono
...
- [x] T9: sv4 — `resultado_consumer` + arranque del hilo daemon en
...
- [x] T10: sv4 — `static/app.js`: modal (encolar sin conflictos; pisar
...
- [x] T11: sv4 — gestión de poison, backend (ampliación 2):
...
- [x] T12: sv4 — gestión de poison, UI (ampliación 2): aviso con recuento
...
- [x] T13: infra — `infra/add_qtransfer_partes.ps1` (colas + `-poison` +
...
- [x] T14: docs — actualizar `docs/ARCHITECTURE.md` (comunicación sv4↔sv5:
...
- [x] T15: Ejecutar `bash harness/init.sh` en verde.
