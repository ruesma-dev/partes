...
def parse_graph_key(graph_key: str) -> Tuple[Optional[GraphCreds], Optional[str]]:
    # Saneo defensivo: un BOM (\ufeff) o espacios al inicio del valor del
    # secreto (p.ej. GRAPH-KEY guardado en Key Vault con BOM) rompian el
    # parseo JSON y degradaban a "raw token", produciendo headers no-ASCII.
    graph_key = (graph_key or "").lstrip("\ufeff").strip()
