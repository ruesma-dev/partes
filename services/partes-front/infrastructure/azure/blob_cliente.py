...
"""Cliente de Blob del portal (sv4) para el hand-off con sv5.

Contenedor 'transfer', con dos prefijos: 'peticiones/' (lo que sv4 pide
registrar) y 'resultados/' (el veredicto por linea que devuelve sv5). El
mensaje de cola lleva solo la referencia: una obra x mes supera de sobra
los 64 KB de un mensaje de Storage Queue.

Lo durable vive en Sigrid (lineas) y en PostgreSQL (columnas `sigrid_*`);
