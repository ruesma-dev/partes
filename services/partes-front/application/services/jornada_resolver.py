...
jornada del DIA, que es la que el portal compara con las horas de cada
dia para avisar de jornada incompleta: el candef efectivo `c` de lunes a
jueves y el resto de la jornada semanal `S` (`max(0, S - 4c)`) el ULTIMO
dia laborable de la semana del trabajador. `S` sale del mapa
`JORNADA_SEMANAL_POR_CANDEF` (espejo del de sv3) o de una excepcion de
`empleado_jornada`. Con `c = 8` y `S = 40` el resto vale 8 y el portal
marca exactamente los mismos dias que antes de F-015.

GEMELO por comportamiento del de sv3
(`services/partes-persistencia/application/services/jornada_resolver.py`):
misma API publica y mismos resultados, con un guardian en la raiz del
monorepo (`tests/test_f015_r19_jornada_resolver_gemelo.py`) que lo
comprueba. Quien cambie la regla cambia LAS DOS copias.
"""
