...
"""F-002 · eleccion de credencial, bucle de cola y composicion de sv4.
...
managed identity, tragarse un `SIGTERM`, o arrancar el portal con el
publisher cableado a una cola distinta de la que consume sv5.
"""
...
def _settings_de_cola(*, con_connection_string: bool):
    """Configuracion minima que lee `_componentes_de_cola`."""
    class SettingsCola:
        colas_connection_string = CS_AZURITE if con_connection_string else None
        colas_account_url = (None if con_connection_string
                             else "https://stpartes.queue.core.windows.net")
        blobs_connection_string = None
        blobs_account_url = (None if con_connection_string
                             else "https://stpartes.blob.core.windows.net")
        cola_transfer = "q-transfer"
        cola_transfer_result = "q-transfer-result"
        blob_transfer = "transfer"
        cola_max_dequeue = 5
        cola_visibility_s = 600
        default_reviewer = "ana"
    return SettingsCola()


def test_f002_r1_el_main_cablea_el_publisher_a_la_cola_correcta(monkeypatch):
    """Si el publisher apuntara a otra cola, sv5 no veria nunca nada."""
    ...
    publisher, cliente_cola = entrypoint._componentes_de_cola(
        _settings_de_cola(con_connection_string=True), repository=object())

    publisher.publicar({"obra": {}, "lineas": []}, usuario="ana")
    assert list(svc_cola.get_queue_client("q-transfer").payloads_enviados[0])         == ["peticion_id", "blob"]
    assert cliente_cola is not None


def test_f002_r12_el_main_arranca_el_consumidor_de_resultados(monkeypatch):
    """Sin este hilo, las lineas se quedarian en 'encolado' para siempre."""
    import main as entrypoint
    ...
    arrancado = {}
    monkeypatch.setattr(entrypoint, "arrancar_consumidor_resultados",
                        lambda **kw: arrancado.update(kw))
    repositorio = object()
    settings = _settings_de_cola(con_connection_string=True)

    entrypoint._componentes_de_cola(settings, repository=repositorio)

    assert arrancado["repository"] is repositorio
    assert arrancado["settings"] is settings
    # El consumidor usa SU cliente, no el del publisher: el SDK no se
    # comparte entre hilos.
    assert arrancado["cola"] is not None
    assert arrancado["blob"] is not None
...
def test_f002_el_main_crea_colas_y_contenedor_en_local(monkeypatch):
    ...
    entrypoint._componentes_de_cola(
        _settings_de_cola(con_connection_string=True), repository=object())
...
def test_f002_en_la_nube_no_se_intenta_crear_la_infraestructura(monkeypatch):
    """En Azure las colas las crea el script de infra; el portal no
    ...
    entrypoint._componentes_de_cola(
        _settings_de_cola(con_connection_string=False), repository=object())
