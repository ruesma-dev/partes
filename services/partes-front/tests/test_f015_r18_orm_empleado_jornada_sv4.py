# tests/test_f015_r18_orm_empleado_jornada_sv4.py
"""R18 · `empleado_jornada` en el ORM de sv4 (y de sv3, byte a byte).
...
def test_f015_r18_sv4_la_tabla_esta_declarada_con_sus_columnas() -> None:
    tabla = Base.metadata.tables["empleado_jornada"]
    assert tuple(c.name for c in tabla.columns) == COLUMNAS


def test_f015_r18_sv4_el_dni_va_indexado() -> None:
...
def test_f015_r18_sv4_los_defaults_de_origen_y_is_active() -> None:
...
def test_f015_r18_sv4_la_vigencia_admite_hasta_abierto() -> None:
    columnas = Base.metadata.tables["empleado_jornada"].columns
    assert columnas["desde"].nullable is False
    assert columnas["hasta"].nullable is True


def test_f015_r18_sv4_el_patron_son_siete_columnas_opcionales() -> None:
    columnas = Base.metadata.tables["empleado_jornada"].columns
...
def test_f015_r18_sv4_parte_registros_no_gana_ni_pierde_columnas() -> None:
...
def test_f015_r18_sv4_create_all_crea_la_tabla() -> None:
    motor = _motor()
    assert "empleado_jornada" in inspect(motor).get_table_names()


def test_f015_r18_sv4_se_puede_insertar_y_leer_una_excepcion() -> None:
...
def test_f015_r18_sv4_se_puede_guardar_un_patron_de_siete_valores() -> None:
    motor = _motor()
    sesion = sessionmaker(bind=motor, expire_on_commit=False, future=True)
    with sesion() as s:
        s.add(EmpleadoJornadaOrm(
