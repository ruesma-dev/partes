# tests/test_f015_r13_ultimo_laborable_sv4.py
...
def test_f015_r13_sv4_el_viernes_es_el_ultimo_laborable_de_la_semana() -> None:
...
def test_f015_r13_sv4_con_el_viernes_festivo_lo_es_el_jueves() -> None:
...
def test_f015_r13_sv4_un_dia_no_laborable_nunca_es_el_ultimo() -> None:
    es_lab = es_laborable_fake({"2026-03-18"})
    assert es_ultimo_laborable(MIERCOLES, es_lab) is False


def test_f015_r13_sv4_el_finde_nunca_es_el_ultimo_laborable() -> None:
...
def test_f015_r13_sv4_solo_mira_los_dias_POSTERIORES_de_su_semana() -> None:
...
def test_f015_r13_sv4_candef_8_da_8_todos_los_dias() -> None:
...
def test_f015_r13_sv4_dia_no_laborable_vale_cero_antes_que_nada() -> None:
    es_lab = es_laborable_fake({"2026-03-20"})
    assert jornada_dia(VIERNES, candef=9.0, minimo=2.0, por_defecto=8.0,
                       mapa=MAPA, es_laborable=es_lab) == 0.0


def test_f015_r13_sv4_sabado_laborable_sin_calendario_vale_el_candef() -> None:
...
def test_f015_r13_sv4_sabado_con_calendario_vale_cero() -> None:
    es_lab = es_laborable_fake()
    assert jornada_dia(SABADO, candef=9.0, minimo=2.0, por_defecto=8.0,
                       mapa=MAPA, es_laborable=es_lab) == 0.0
...
def test_f015_r13_sv4_el_detalle_explica_de_donde_sale_la_jornada() -> None:
...
def test_f015_r13_sv4_el_detalle_de_un_dia_normal() -> None:
    det = detalle_jornada_dia(
...
def test_f015_r13_sv4_el_detalle_de_un_candef_fuera_del_mapa() -> None:
    det = detalle_jornada_dia(
...
def test_f015_r13_sv4_jornada_dia_es_azucar_de_detalle_jornada_dia() -> None:
    for d in SEMANA:
        es_lab = es_laborable_fake()
        assert jornada_dia(
...
def test_f015_r13_sv4_con_S_igual_a_5c_no_se_consulta_el_resto_de_la_semana() -> None:
...
def test_f015_r13_sv4_como_mucho_se_consultan_los_LV_de_la_semana() -> None:
    consultadas: list[str] = []
