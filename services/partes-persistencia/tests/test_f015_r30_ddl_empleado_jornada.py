# tests/test_f015_r30_ddl_empleado_jornada.py
...
def test_f015_r30_hay_un_alter_por_columna_no_primaria() -> None:
...
def test_f015_r30_el_ddl_de_la_tabla_es_el_esperado() -> None:
...
def test_f015_r30_el_indice_del_dni_se_crea() -> None:
    assert (
...
def test_f015_r30_la_tabla_aporta_exactamente_19_sentencias() -> None:
    """18 columnas no primarias + 1 indice: ni una mas."""
    assert len(_de_la_tabla()) == 19


def test_f015_r30_ninguna_columna_not_null_se_queda_sin_default() -> None:
...
def test_f015_r30_toda_sentencia_de_la_tabla_es_idempotente() -> None:
    for sentencia in _de_la_tabla():
        assert "IF NOT EXISTS" in sentencia, sentencia


def test_f015_r30_el_ddl_de_la_tabla_es_solo_aditivo() -> None:
    for sentencia in _de_la_tabla():
        for verbo in ("ALTER COLUMN", "DROP ", "RENAME ", "DELETE ", "UPDATE "):
            assert verbo not in sentencia.upper(), sentencia


def test_f015_r30_las_demas_tablas_no_ganan_ni_pierden_sentencias() -> None:
