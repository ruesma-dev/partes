# tests/test_f015_r12_candef_invalido.py
...
def test_f015_r12_candef_invalido_da_8_en_todo_laborable(candef) -> None:
...
@pytest.mark.parametrize("candef", INVALIDOS)
def test_f015_r12_el_detalle_lo_declara_como_candef_efectivo_8(candef) -> None:
...
@pytest.mark.parametrize("candef", INVALIDOS)
def test_f015_r12_candef_valido_sigue_diciendo_que_no(candef) -> None:
    """El KPI del portal marca 'asignado' leyendo de aqui: no cambia."""
    assert candef_valido(candef, minimo=2.0) is False


def test_f015_r12_una_jornada_por_defecto_distinta_manda() -> None:
